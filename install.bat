...
for /f "delims=" %%i in ('where /r "C:\" clang.exe 2^>nul & where /r "%INSTALL_PATH%" clang.exe 2^>nul') do set CLANG_PATH=%%i
if defined CLANG_PATH (
    for %%i in ("%CLANG_PATH%") do set CLANG_DIR=%%~dpi
    setx PATH "%PATH%;%CLANG_DIR%" /M
    echo Added %CLANG_DIR% to PATH
) else (
